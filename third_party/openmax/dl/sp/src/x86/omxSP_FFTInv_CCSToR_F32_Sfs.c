 ...
 * A[k] = (X[k] + X'[N/2 - k]) / 2
 * B[k] = (X[k] - X'[N/2 - k]) / 2 * W[k], (W = exp(j*2*PI*k/N); k = 0, ..., N/2 - 1)
...
static void RevbinPermuteInvSse(OMX_F32 *in,
                                OMX_F32 *out,
                                OMX_F32 *twiddle,
                                OMX_INT n) {
  OMX_INT i;
...
OMXResult omxSP_FFTInv_CCSToR_F32_Sfs(const OMX_F32 *pSrc, OMX_F32 *pDst,
  ...
  tw = pFFTStruct->pTwiddle;

  if (n < 8)
    RevbinPermuteInv(in, buf, tw, n);
  else
    RevbinPermuteInvSse(in, buf, tw, n);

  if (n_by_2 < 16) {
    buf = (OMX_F32*)x86SP_F32_radix2_kernel_OutOfPlace(
        buf,
        pFFTStruct->pBuf2,
        buf,
        tw,
    ...
    buf = (OMX_F32*)x86SP_F32_radix4_kernel_OutOfPlace_sse(
        buf,
        pFFTStruct->pBuf2,
        buf,
        tw,
