...
void x86SP_FFT_CToC_FC32_Inv_Radix4_ms_sse(
  ...
  for (set = 0; set < set_count; set += 4) {
    OMX_F32 * in0 = in + set;
    OMX_F32 *in1 = in0 + set_count;
    OMX_F32 *in2 = in1 + set_count;
    OMX_F32 *in3 = in2 + set_count;
  ...
  for (grp = 1; grp < sub_size; ++grp) {
    OMX_F32 *tw1 = twiddle + grp * step;
    OMX_F32 *tw2 = tw1 + grp * step;
    OMX_F32 *tw3 = tw2 + grp * step;
